...
> 📍 开源免费的 Open WebUI 管理员助手
> 
## 简介

Bouncer 是一个专为 [Open WebUI](https://openwebui.com/) 设计的网关管理过滤器。通过精细的规则配置，可以实现用户分级、模型分组、访问控制、频率限制、关键词过滤等功能，确保你的 LLM 应用安全、可控、高效。
...
### 1. 安装函数
打开你的 Open WebUI 管理员设置>函数>添加新函数

### 2. 使用配置编辑器
在浏览器打开 [Bouncer WebUI](https://bouncer-webui.pages.dev/)，使用友好的可视化编辑器配置 Bouncer。

### 3. 生成配置 JSON
编辑器会实时生成 JSON 配置，点击「⬇ 导出」或「⧉ 复制」获取配置文本。

### 4. 粘贴到 Open WebUI
在 Open WebUI 中，进入函数管理，找到 Bouncer 的 `config_json` 字段，粘贴生成的 JSON 配置。
