...
class Filter:
    class Valves(BaseModel):
        config_json: str = Field(
            ...
            description="Bouncer configuration (JSON/JSONC). Supports // and /* */ comments.",
        )
    ...
    def _log(self, cfg: Dict[str, Any], level: str, msg: str, data: Any = None) -> None:
        ...
        prefix = "[Bouncer]"
        if level == "Bouncer":
            if logging_cfg.get("bouncer_log", False):
    ...
    async def inlet(
        ...
        if cfg.get("exemption", {}).get("enabled", False) and self._email_in_list(
            ...
            self._log(cfg, "Bouncer", "Exempted User", self._normalize_email(email))
            return body
        ...
        if isinstance(cfg.get("user_groups"), list) and len(cfg["user_groups"]) > 0:
            ...
            if is_limited:
                self._log(cfg, "Bouncer", "Rate Limit Hit", limit_reason)
